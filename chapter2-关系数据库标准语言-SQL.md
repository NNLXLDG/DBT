...
## 关系的性质
“关系的性质”指的是：关系模型中，一张关系（表）在逻辑上必须满足的基本特性。

+ 列是同质的（同一列数据类型相同）
+ 每个分量都是不可再分的原子值（原子性）,关系中的每个属性值都是不可再分的。
+ 行（元组）之间没有顺序
+ 列（属性）之间没有顺序
+ 不允许有完全相同的元组（行唯一性）
+ 属性名必须唯一
...
关系是符合某一关系模式的具体数据集合。由若干元组（行）组成内容随时间变化.关系是关系模式在某一时刻的具体体现，关系必须符合其对应的关系模式。


## 关系数据模型的数据结构、数据操作和完整性约束

关系数据模型由数据结构、数据操作和完整性约束三部分组成。其中数据结构是关系，用二维表表示；数据操作包括查询和插入、删除、修改等操作；完整性约束用于保证数据的正确性和一致性，主要包括实体完整性、参照完整性和用户完整性。


关系数据模型的数据结构是关系，即由二维表表示的数据结构。	一个关系对应一张表,行称为元组 , 列称为属性 , 属性取值来自某一域。关系数据模型的数据操作主要包括查询和更新操作。完整性约束用于保证数据库中数据的正确性和一致性。

+ 实体完整性：主键不能为空，主键唯一，通过 PRIMARY KEY 实现
+ 参照完整性，外键要么为空，要么必须在被引用表中存在，通过 FOREIGN KEY 实现
+ 由用户根据业务规则自行定义，如取值范围、非空约束等，通过 CHECK / NOT NULL / DEFAULT / 触发器 实现
